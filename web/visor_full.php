<?php
// =====================================================
// visor_full.php — Obre les dades de la Virko
// dins del visor ONLYOFFICE integrat a la web
// =====================================================
...
// Escrivim el CSV amb les dades dels sensors
$fp = fopen($ruta, 'w');
...
// URL PÚBLICA per accedir al fitxer des d'ONLYOFFICE (IMPORTANT: NO USAR 'web' NI 'localhost')
$url_fitxer = "http://74.242.168.29/fulls/" . $nom_fitxer;

$rol         = $_SESSION['rol'];
$nom_complet = $_SESSION['nom_complet'];

// Paràmetre mode de la URL (per a l'app Qt de l'Amin)
// mode=lectura → només lectura | sense paràmetre → edició
...
<head>
  ...
  <title>VirKO — <?= htmlspecialchars($nom_aula) ?></title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    header {
      background: linear-gradient(to right, #ffffff 0%, #ffffff 10%, #1a3a6b 50%, #0a1628 80%);
      padding: 0 40px;
      display: flex;
      align-items: center;
...
<div class="info-bar">
  ...
  <span class="mode-tag <?= ($mode_onlyoffice === 'view') ? 'mode-lectura' : 'mode-edicio' ?>">
    <?= ($mode_onlyoffice === 'view') ? '👁 Només lectura' : '✏️ Edició' ?>
  </span>
  <div class="dreta">
...
<script src="http://74.242.168.29:8081/web-apps/apps/api/documents/api.js"></script>
...
var docEditor = new DocsAPI.DocEditor("onlyoffice-editor", {
    document: {
        fileType: "csv",
        key: "<?= $mac . '_' . time() ?>",
        title: "Dades_<?= addslashes($nom_aula) ?>.csv",
        url: "<?= $url_fitxer ?>", // URL pública generada arriba
        options: {
            "delimiter": ",",
            "encoding": 65001 
        }
    ...
    editorConfig: {
        ...
        customization: {
            ...
            chat: false,
            help: false,
            logo: { image: "http://74.242.168.29/logo.png", url: "http://74.242.168.29" }
        }
    },
    width: "100%",
    height: "100%"
